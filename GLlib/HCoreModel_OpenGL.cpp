...
sModelMaterial_OpenGL *CHCoreModel_OpenGL::s_pMaterialOverwrite = NULL;
GLuint CHCoreModel_OpenGL::s_ShaderOverwrite = 0;
GLuint CHCoreModel_OpenGL::s_TextureOverwrite[MAX_NUM_TEXTURES] = {0,0,0};
GLuint CHCoreModel_OpenGL::s_MapOverwrite[MAX_NUM_TEXTURES] = {MAP_NOOVERWRITE, MAP_NOOVERWRITE, MAP_NOOVERWRITE};
...
void sModelMaterial_OpenGL::Submit(sModelVertexChunk_OpenGL *pVChunk, UINT mask)
{
	...
	{
		int map = CHCoreModel_OpenGL::s_MapOverwrite[t]==MAP_NOOVERWRITE ? m_MapChannel[t] : CHCoreModel_OpenGL::s_MapOverwrite[t];
		GLuint texid = CHCoreModel_OpenGL::s_TextureOverwrite[t] ? CHCoreModel_OpenGL::s_TextureOverwrite[t] : m_Textures[t];
	...
	GLuint shader = CHCoreModel_OpenGL::s_ShaderOverwrite;
...
void CHCoreModel_OpenGL::Release(void)
{
	if ( m_pMeshArray )
	{
		delete [] m_pMeshArray;
...
bool CHCoreModel_OpenGL::ConvertToOpenGLModel(CHCoreModel *pModel)
{
	...
	if ( m_iNumMaterials )
	{
		...
		for ( i=0; i<m_iNumMaterials; i++, target++, source++ )
		{
			...
			for ( j=0; j<MAX_NUM_TEXTURES; j++ )
			{
				...
				{
					sprintf(szTextureName, "%s%s", CHCoreModel::GetTexturePath(), source->m_szTexture[j]);
					if ( source->m_MapChannel[j]==MAP_CUBEMAP )
					{
						target->m_Textures[j] = HCoreLoadCubemapTexture_OpenGL(szTextureName);
					}
					else
					{
						target->m_Textures[j] = HCoreLoadTexture_OpenGL(szTextureName);
					}
				}
				else
				{
...
void CHCoreModel_OpenGL::RenderMesh(bool bSubmitMaterial)
{
	Render(bSubmitMaterial ? 0xff : 0);
}

void CHCoreModel_OpenGL::Render(UINT mask)
{
